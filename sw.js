// TipidMix service worker — offline-first PWA channel.
// Strategy: pre-cache the app shell on install; cache-first for same-origin
// GETs with background refresh (stale-while-revalidate). The optimizer is
// 100% client-side, so once cached the app is fully functional with zero
// signal — the PH job-site reality. The mix math is 100% client-side.
// Bump VERSION on every release (CI stamps it via the release script).
const VERSION = 'tipidmix-v0.1.2';
